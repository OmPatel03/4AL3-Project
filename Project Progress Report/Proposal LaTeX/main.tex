...
\begin{itemize}
    ...
    \item \textbf{XGBoost}, a state-of-the-art gradient-boosting framework offering strong regularization and flexible tree construction \citep{Chen:2016}.
\end{itemize}
...
Om led the development of the predictive models, including data preprocessing, feature engineering, and implementation of the gradient-boosted tree classifiers. He conducted the ablation studies, implemented probability calibration, and integrated the feature variants used in the final experiments. Om also contributed to the design of the A~$\rightarrow$~B~$\rightarrow$~C compatibility pipeline and authored major portions of the technical sections of the report.

Gregory was responsible for exploratory data analysis, dataset cleaning, and construction of participant-level profile representations used throughout the project. He identified relevant survey features, assisted in organizing the pairing-based input format, and produced visualizations that informed model selection and evaluation. Gregory also contributed to interpreting model outputs and preparing supporting figures for the final report.

Alvin contributed to the experimental design and evaluation methodology, helping define baseline models, select appropriate performance metrics, and structure the train/validation/test splitting strategy. He assisted with documentation, report writing, and result interpretation, ensuring that the analysis remained clear, coherent, and aligned with the project goals.
